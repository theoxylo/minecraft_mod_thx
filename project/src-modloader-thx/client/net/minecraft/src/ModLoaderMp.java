...
public class ModLoaderMp
{
    ...
    private static Map modsById = new HashMap();
    ...
    {
        //Log("<<<<< ModLoaderMP.HandleAllPackets - packet received: " + packet230modloader);
        
        // this speeds up delivery for the most common packets
        BaseModMp cachedMod = (BaseModMp) modsById.get(new Integer(packet230modloader.modId));
        if (cachedMod != null)
        {
            cachedMod.HandlePacket(packet230modloader);
            return;
        }
        ...
        else
        {
            for (int i = 0; i < ModLoader.getLoadedMods().size(); i++)
            {
                ...
                // cache mod by id for next time, read at beginning of method
    ...
    public static void RegisterNetClientHandlerEntity(Class class1, boolean hasOwner, int i)
    {
        if (!hasInit)
        {
            init();
        ...
        else
        {
            ...
            netClientHandlerEntityMap.put(Integer.valueOf(i), new NetClientHandlerEntity(class1, hasOwner));
        }
    }
    ...
    private static void sendPacket(Packet230ModLoader packet230modloader)
    {
        ...
        {
            Log(">>>>> Sending packet: " + packet230modloader);
