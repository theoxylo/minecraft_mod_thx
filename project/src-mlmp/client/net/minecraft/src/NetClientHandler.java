...
public class NetClientHandler extends NetHandler
{
    ...
    public void handleVehicleSpawn(Packet23VehicleSpawn par1Packet23VehicleSpawn)
    {
        ...
        NetClientHandlerEntity var18 = ModLoaderMp.handleNetClientHandlerEntities(par1Packet23VehicleSpawn.type);
        Entity var11;
        ...
        if (var18 != null)
        {
            try
            {
                var8 = (Entity)var18.entityClass.getConstructor(new Class[] {World.class, Double.TYPE, Double.TYPE, Double.TYPE}).newInstance(new Object[] {this.worldClient, Double.valueOf(var2), Double.valueOf(var4), Double.valueOf(var6)});

                if (var18.entityHasOwner)
                {
                    Field var20 = var18.entityClass.getField("owner");

                    if (!Entity.class.isAssignableFrom(var20.getType()))
                    {
                        throw new Exception(String.format("Entity\'s owner field must be of type Entity, but it is of type %s.", new Object[] {var20.getType()}));
                    }
                    ...
                    {
                        if (!var20.getType().isAssignableFrom(var11.getClass()))
                        {
                            throw new Exception(String.format("Tried to assign an entity of type %s to entity owner, which is of type %s.", new Object[] {var11.getClass(), var20.getType()}));
                        }

                        var20.set(var8, var11);
                    }
                }
            }
            catch (Exception var16)
            {
                ModLoader.getLogger().throwing("NetClientHandler", "handleVehicleSpawn", var16);
                ModLoader.throwException(String.format("Error initializing entity of type %s.", new Object[] {Integer.valueOf(par1Packet23VehicleSpawn.type)}), var16);
                return;
            }
        }
